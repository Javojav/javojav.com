...
function draw() {
	if (stop == false) {
		...
		reloadak();
		wait++;
		if (ammo.total < 0 && ammo.ingun < 0) {
			ammo.ingun = 0;
			ammo.total = 0;
		}
		if (akammo.total < 0 && akammo.ingun < 0) {
			akammo.ingun = 0;
			akammo.total = 0;
		}
		fill(255)
		textSize(10);
		text(ammo.ingun + "/" + ammo.total, width - 100, height - 10);
		text(akammo.ingun + "/" + akammo.total, width - 200, height - 10);
